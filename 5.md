...
4. Création de composants simples
...
Dans cette partie, nous allons découvrir quelques fonctions (fournies par le langage JavaScript et le DOM) et bonnes pratiques à connaître pour réaliser des composants sur le Web.
...
### Sélecteur par type d'élément: `getElementsByTagName()`
...
- doit définir des règles et contraintes éventuelles que devront respecter les intégrateurs du composant. (ex: *toutes les images à afficher dans la galerie doivent être des balises `<img>` portant la classe `carousel-img`*)
...
- une démonstration "live" du composant, pour le tester;
- un exemple simple d'intégration de ce composant;
- les instructions permettant d'intégrer rapidement ce composant;
- la description des fonctions et/ou paramètres fournis par le composant;
