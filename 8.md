...
1. Effectuer une requête `GET` avec `XMLHttpRequest` ([Slides du TP](./slides/13-ajax/index.html))
  - Exercice: afficher la météo de la ville de l'utilisateur
2. Effectuer une requête `POST` avec `XMLHttpRequest` ([Slides du TP](./slides/14-ajax2/index.html))
