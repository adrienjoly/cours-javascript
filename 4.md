...
Afin de mettre en pratique l'association d'un programme JavaScript à une page HTML, l'accès à un élément en JavaScript, et la récupération de la valeur d'un champ de saisie, vous aller:

1. Créer un répertoire `JS-FORM-1`;
2. Dans ce répertoire, créer un fichier `index.html` valide contenant deux champs de saisie, tel que définis dans l'exemple ci-dessus (avec identifiants `nom` et `prenom`);
3. Toujours dans ce répertoire, créer un fichier `index.js` contenant un programme JavaScript qui affichera avec `alert()` la valeur du premier champ de la page, puis celle du deuxième champ;
4. Associez votre programme JavaScript (fichier `index.js`) à votre page HTML (fichier `index.html`), de manière à ce qu'il soit exécuté quand on ouvre le page, et vérifier que les deux valeurs sont bien affichées dans des `alert` lors du chargement de la page dans votre navigateur;
...
Pour l'instant, nous allons employer la méthode la plus simple, qui consiste à affecter une fonction à la propriété d'un élément correspondante à l'événement choisi.
...
En suite, y associer un fichier JavaScript qui permettra à l'utilisateur, à chaque fois qu'il cliquera sur le bouton `mon-bouton`, d'obtenir le résultat de l'addition des nombres qu'il aura saisis dans les champs `premier-nombre` et `deuxieme-nombre`, dans le champ `resultat`.
