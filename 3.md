...
Dans le début de ce chapitre nous avons vu que les tableaux permettaient de stocker une liste ordonnées de valeurs. Et que les éléments d'un tableau étaient indexés par numéro.
...
Les objets permettent aussi de stocker des valeurs. Par contre, les éléments stockés dans un objet sont appelés **propriétés**, et sont indexés par des chaînes de caractères.
...
Contrairement aux tableaux, le nom de chaque propriété (appelé **clé**, ou **key** en anglais) doit être mentionné explicitement pour chaque valeur, en séparant les deux par `:` (deux points).

Chaque définition de propriété doit être ponctué d'une virgule.

Et les règles d'indentation s'applique aussi ici, car les propriétés sont définies entre accolades.
...
Comme pour les tableaux, l'adressage par crochets a un avantage intéressant pour les objets: il permet d'accéder à un élément dont la clé est stockée dans une variable.
...
La notation pointée est plus concise et lisible, mais impose de mentionner littéralement/explicitement la clé.
...
Contrairement aux noms de variables autorisés en JavaScript, il est possible 
d'inclure des espaces et des caractères spéciaux dans les clés de propriétés. Par contre, il faut pour cela utiliser des apostrophes pour définir et adresser ces propriétés:
...
Comme pour les tableaux, la valeur d'une propriété peut être de n'importe quel type: `string`, `number`, `boolean`, `null`, `undefined`, `object` ou `function`.

A noter que les tableaux sont en fait de type `object`, même s'ils ont des caractéristiques particulières. (ex: indexation numérique, fonctions spécifiques comme `slice`, etc...)
...
Cet objet peut être représenté efficacement sous forme d'un arbre.

Pour accéder au premier membre du groupe `maitresJedi`, on peut utiliser la notation pointée ou les crochets.
...
2. ou itérer sur le tableau des clés, à l'aide de la fonction `Object.keys()`.
...
Dans notre exemple de code ci-dessus, la variable `cle` prendra donc la valeur d'une clé de l'objet `mesAmis`, à chaque itération.

### Énumérer les propriétés d'un objet (suite)

Lorsqu'elle est appelée en passant un objet en paramètre, la fonction `Object.keys()` retourne un tableau contenant les clés/noms de chaque propriété de ce tableau.
...
- recherche d'un numéro de téléphone en saisissant un nom,
...
Pour permettre à l'utilisateur de choisir la fonctionnalité qu'il souhaite utiliser, lui demander de saisir la première lettre de la fonction (ex: `'r'` pour rechercher, `'l'` pour afficher la liste, `'a'` pour ajouter, et `'s'` pour supprimer).
