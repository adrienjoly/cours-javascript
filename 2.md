...
Comme en mathématiques, une fonction transformae des paramètres (en "entrée") en une valeur de résultat (la "sortie"), lorsqu'elle est appelée. Avant de pouvoir l'appeler, on la définit par une suite d'instructions qui déterminera cette valeur de résultat, en fonction des paramètres qui lui seront passés. 
...
```js
// definition de la fonction multiplierParDeux()
function multiplierParDeux (nombre) {
  return nombre * 2;
}
